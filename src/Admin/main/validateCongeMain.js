...
const ManageConges = () => {
  ...
  // Function to download all conges as Excel
  const downloadConges = async () => {
    try {
      const token = localStorage.getItem("token"); // Get token from localStorage

      // Make a GET request to the export API endpoint
      const response = await axios.get("http://localhost:8082/conges/exportConges", {
        headers: {
      ...
      link.download = "conges.xlsx"; // Specify the file name
      link.click(); // Trigger the download
    } catch (error) {
      console.error("Error downloading Conges Excel file:", error);
  ...
  return (
    <div className="container mt-5">
      ...
      <button
        ...
        onClick={downloadConges}
      >
        Download All Conges (Excel)
      </button>
